...
本文件说明稳定的使用边界，可直接用于模块职责、架构和常规实现决策。只有本文及关联专题未覆盖、当前调用需要精确签名或运行语义，或项目构建/索引与本文冲突时，才查询依赖索引返回的实际解析构件及最小资料来确认公开类型和成员签名；不得以其它项目旧代码替代查询，也不追踪 PluginBase 的隐式版本字符串。
...
`library`、`misc` 等名称并不等于可随意调用其中任意类。本文或关联专题未列出的成员、当前调用的重载差异和版本敏感语义仍须通过依赖索引查询实际解析构件；不得为了重复确认本文已说明的设计边界而读取无关框架源码，也不以隐式版本字符串替代构件查询。
