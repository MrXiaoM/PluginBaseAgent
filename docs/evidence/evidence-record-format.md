...
- 来源：<运行字节码/sources/Javadoc/元数据/锁定源码/临时 Vineflower 反编译>
- 位置：<GAV> | SHA-256 <哈希> | <归档内路径或 Javadoc 锚点>#<类/成员>
...
- 目标：Spigot API；`<精确版本>`。
- 来源：运行字节码与 sources。
- 位置：`<Spigot GAV>` | SHA-256 `<哈希>` | `sources.jar:<归档内路径>`#`<成员签名>`。
- 证据：`<实际类声明、可取消接口或方法签名>`。
...
- 目标：PluginBase；`<精确版本>`。
- 来源：运行字节码与 sources。
- 位置：`<PluginBase paper GAV>` | SHA-256 `<哈希>` | `sources.jar:.../PaperFactory.java`#`createItemEditor`、`createInventoryFactory`。
...
- 结论：不在最低支持版本使用 `<方法>`。
- 目标：Spigot API；`<最低版本>`。
- 来源：运行字节码、sources 与 Javadoc。
- 位置：`<Spigot GAV>` | SHA-256 `<哈希>` | `sources.jar:<归档内路径>`#`<类>`。
...
1. 用的是什么精确版本？
2. 资料在哪里？别人能否依据当前项目索引中的 GAV、哈希与归档内路径，或明确 URL 复查？
3. 实际调用的签名/行为是什么？
4. 它为什么适合本次任务？
