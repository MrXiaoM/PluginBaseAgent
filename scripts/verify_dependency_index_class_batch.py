...
def bytecode_fixture() -> bytes:
    ...
    pool = [
        utf8("example/bytecode/Sample"), b"\x07\x00\x01",
...
def main() -> int:
    ...
    assert api_total == API_COUNT + 5, api_total
    ...
    print(f"通过：Gradle 实时输出与资料路径、Javadoc 单页摘要映射、分批写入 {CLASS_COUNT + 2} 个类与 {API_COUNT} 条源码 API、5 条字节码 API；两个 FTS 各命中 1 条，类名进度覆盖 3 个批次。")
    return 0
